...
local function UpdateSelection(self)
  for i, itemFrame in ipairs(self.itemFrames) do
    if itemFrame.selected then
      itemFrame.bgFrame:SetBackgroundColor(0, 0, 0.5, 1)
    else
...
local function SetItems(self, items, values, levels)
  ...
  -- clear selection without triggering event
...
local function AddSelectedIndex(self, index)
...
local function RemoveSelectedIndex(self, index)
  if self.selectionMode ~= "multi" then
    error("List is not in multi-select mode.")
  end
...
local function SetSelectedIndices(self, indices)
  if self.selectionMode ~= "multi" then
    error("List is not in multi-select mode.")
  end
...
local function SetSelectedItems(self, items)
  if self.selectionMode ~= "multi" then
    error("List is not in multi-select mode.")
  end
  ...
  self:SetSelectedIndices(indices)
end
...
local function SetSelectedValues(self, values)
  if self.selectionMode ~= "multi" then
    error("List is not in multi-select mode.")
  end
